...
const ReportsPage: React.FC = () => {
    ...
    const { bungalows, reservations, fetchBungalows, fetchReservations, isLoading } = useData();
    ...
    useEffect(() => {
        fetchBungalows();
        fetchReservations();
    }, [fetchBungalows, fetchReservations]);
    ...
    const reportData = useMemo(() => {
        ...
        const yearlyConfirmedReservations = reservations.filter(r => new Date(r.startDate).getFullYear() === currentYear && r.status === ReservationStatus.Confirmed);
        ...
        return { totalRevenue, totalReservations, occupancyRate, chartData, monthlyRevenueData };
    ...
    const handleGenerateSummary = async () => {
        ...
        try {
            ...
            alert("Une erreur est survenue lors de la génération du résumé IA.");
        } finally {
            setSummaryLoading(false);
        }
    };
    
    const showLoading = (isLoading.bungalows || isLoading.reservations) && reservations.length === 0;
    ...
    return (
        <div>
                ...
                {settings.moduleStatus.ai && (
                    <Button onClick={handleGenerateSummary} disabled={isSummaryLoading} variant="secondary">
                        {isSummaryLoading ? (
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
                        )}
            ...
            <div className="mt-6">
                <ReportFilters onDateChange={handleDateChange} onPresetChange={handlePresetChange} startDate={dateRange.startDate} endDate={dateRange.endDate} activePreset={activePreset} />
            </div>
            
            {showLoading ? (
                 <div className="text-center py-12">Chargement des données du rapport...</div>
            ) : (
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                        <ReportCard title="Revenu Total" value={`${reportData.totalRevenue.toLocaleString('fr-FR')} ${settings.financial.currency}`} description={`sur ${reportData.totalReservations} réservation(s)`} />
                        <ReportCard title="Taux d'Occupation" value={`${reportData.occupancyRate.toFixed(1)}%`} description="Basé sur les nuits confirmées" />
                        <ReportCard title="Réservations Confirmées" value={reportData.totalReservations} description="Dans la période sélectionnée" />
                    </div>
            ...
            {isSummaryModalOpen && (
                <Modal isOpen={isSummaryModalOpen} onClose={() => setSummaryModalOpen(false)} title="Résumé de l'Analyse IA" footer={<Button onClick={() => setSummaryModalOpen(false)}>Fermer</Button>}>
