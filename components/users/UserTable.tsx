...
const UserTable: React.FC<UserTableProps> = ({ users, onEdit, onDelete, onViewAuditLog }) => {
                        ...
                        {users.map((user) => (
                            <tr key={user.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td className="px-6 py-4 text-right space-x-2">
                                    <button onClick={() => onViewAuditLog(user)} className="font-medium text-primary-600 dark:text-primary-500 hover:underline">Logs</button>
                                    {canWrite && (
                                        <>
                                            ...
                                            {currentUser?.id !== user.id && (
                                                <button onClick={() => onDelete(user.id)} className="font-medium text-red-600 dark:text-red-500 hover:underline">Supprimer</button>
                                            )}
                                        </>
                                    )}
                                </td>
