...
const MonthlyRevenueChart: React.FC<MonthlyRevenueChartProps> = ({ data, currency }) => {
    ...
    const magnitude = Math.pow(10, Math.floor(Math.log10(maxRevenueRaw || 1)));
    const maxRevenue = maxRevenueRaw > 0 ? Math.ceil(maxRevenueRaw / magnitude) * magnitude : 50000;
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow mb-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Revenus encaissés mensuels (Année en cours)</h3>
            <div className="flex" style={{ height: '300px' }}>
                <div className="flex flex-col justify-between text-xs text-gray-500 dark:text-gray-400 pr-4 text-right">
                    {yAxisLabels.map(label => (
                        <div key={label}>{`${(label / 1000).toLocaleString('fr-FR')}k`}</div>
                    ))}
                </div>
